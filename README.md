...
Scan output is written to `scans/<target>_<timestamp>/logs/scan.log` during the run. The terminal shows a clean progress display.

### Output files

| File | Description |
|---|---|
| `ThreatMap_<target>_<timestamp>.json/.txt/.html` | Report outputs generated by `ai_reporter.py` |
| `scans/<target>_<timestamp>/report/evidence_*.json` | HTTP evidence probe artifacts |
| `scans/<target>_<timestamp>/logs/scan.log` | Full verbose scan output for review |
...
├── ai_reporter.py          # Primary JSON/TXT/HTML report builder
